%% Random task allocation along a given path 
% assuming path{1} ~= robot.node

function [trajectory, fval, analysis] = random_task_allocator(robot, charger, path)
    num_inputs = 10;
    ...
    %% Find the best task allocation using the fitness function
    ...
    fitness(ones(size(path)));
    cache.Nodes.flag(1) = 0;

    % get final utilities
    cache = rmnode(cache, cache.Nodes.Name(cache.Nodes.flag == 3));
