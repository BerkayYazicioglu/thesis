...
function schedule = charger_dogwalk_path_planner(charger, time, map, robots)

    % find the current center of all agent locations
    locsx = charger.world.X([cellfun(@(x) str2num(x.node), [robots {charger}])]);
    locsy = charger.world.Y([cellfun(@(x) str2num(x.node), [robots {charger}])]);
    goal = [mean(locsx) mean(locsy)];
