function single_heatmap_analysis(mission, gui, robot_id)
    ...
    % map
    % ==================================================================
    robot = mission.robots(1);
    map1 = imagesc(ax, ...
                 [robot.world.X(1,1) robot.world.X(1,end)], ...
                 [robot.world.Y(1,1) robot.world.Y(end,1)], ...
                 zeros([robot.world.size(), 3]));
    c = 0.4 * ones(robot.world.size());
    c(str2double(robot.map.Nodes.Name)) = 0;
    [bin, ~] = conncomp(robot.map);
    accessible = 0 * bin;
    robot_bin = bin(robot.map.findnode(robot.node));
    accessible = accessible | (bin == robot_bin);
    c(cellfun(@str2double, robot.map.Nodes.Name(~accessible))) = 1;
    set(map1, 'AlphaData', c);
    % ==================================================================
    ...
    accessible = accessible | (bin == robot_bin);
    c(cellfun(@str2double, robot.map.Nodes.Name(~accessible))) = 1;
    set(map2, 'AlphaData', c);

    charger_path = mission.charger.history.node;
    charger_plot = plot(ax, ...
                    mission.world.X(str2double(charger_path)), ...
                    mission.world.Y(str2double(charger_path)), ...
                    '-r', 'LineWidth', 1.5);
    charger_pos = scatter(ax, ...
        mission.world.X(str2double(charger_path(end))), ...
        mission.world.Y(str2double(charger_path(end))), ...
        250, ...
